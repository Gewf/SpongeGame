...
namespace OpenTKPlatformerExample
{
	class Input
	{
        ...
        //Gamepad
        public static bool right, left, up, down, A, B;
        public static bool lastB,lastA; //Check if B is just pressed, for fireflower
        ...
        public static void EarlyUpdate()
        {
            ...
            {
                right = GamePad.GetState(0).DPad.IsRight;
                left = GamePad.GetState(0).DPad.IsLeft;
                up = GamePad.GetState(0).DPad.IsUp;
                down = GamePad.GetState(0).DPad.IsDown;
                A = GamePad.GetState(0).Buttons.A.HasFlag(ButtonState.Pressed);
                B = GamePad.GetState(0).Buttons.B.HasFlag(ButtonState.Pressed);
